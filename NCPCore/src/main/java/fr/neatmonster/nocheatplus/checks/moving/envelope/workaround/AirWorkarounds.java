 ...
 *  - Each workaround has to have proper documentation. Emphasis on the "why" the workaround is needed in the first place.  <br>
...
public class AirWorkarounds {
    ...
    private static boolean oddLevitation(final MovingData data, final Player player, final PlayerLocation from, final boolean fromOnGround) {
        ...
        return  
                ...
                || pData.getClientVersion().isOlderThan(ClientVersion.V_1_9) && thisMove.yDistance > 0.0
    ...
    public static boolean checkPostPredictWorkaround(final MovingData data, final boolean fromOnGround, final boolean toOnGround, final PlayerLocation from, 
                                                     final PlayerLocation to, final double predictedDistance, final Player player) {
    	final PlayerMoveData lastMove = data.playerMoves.getFirstPastMove();
        final PlayerMoveData thisMove = data.playerMoves.getCurrentMove();
        ...
        if (thisMove.hasLevitation) {
            return oddLevitation(data, player, from, fromOnGround);
        }
        ...
        return  
                ...
                || from.isHeadObstructed(0.2, false) && thisMove.yDistance != 0.0
                && data.ws.use(WRPT.W_M_SF_HEAD_OBSTRUCTION)
                ...
                // Happens when stepping really small heights differences (i.e.: try moving back and forth enough times between dirt and a dirth path block without this workaround)
                // Also happens when sprinting over small, 1-block big gaps (the game does allow players to do so)
                ...
                // TODO: his needs to be handled in a more decent way in RichEntityLocation. Will be removed.
